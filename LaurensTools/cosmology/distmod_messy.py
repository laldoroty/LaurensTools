...
class HubbleDiagram(object):
    ...
    def __init__(self,data,model='tripp',cosmo=FlatLambdaCDM(H0=70, Om0=0.3)):
        acceptable_models = ['tripp','salt','frni','pEW 4000','He2018','Aldoroty2022']
        self.data = data
        self.model = model
        self.cosmo = cosmo
    ...
    def test_dm(self,train_pars,test,train):
        ...
        elif self.model=='frni' or self.model=='pEW 4000':
            M,a,b = train_pars
            if self.model=='frni':
                mu = test['bmax'] - M - a*(test['frni']) - b*(test['c'])
            elif self.model=='pEW 4000':
                mu = test['bmax'] - M - a*(test['pEW 4000']) - b*(test['c'])
    ...
    def minimize(self,verbose=False,return_fit=False):
        ...
        def userfunc(m, n, theta, private_data=self.input_data):
            if self.model=='tripp':
                ...
                M,a,b = theta
                BMAX, EBMAX, PEW, EPEW, C, EC, EVPEC, MU = private_data["bmax"], private_data["ebmax"],private_data["pEW 4000"],private_data["epEW 4000"],private_data["c"], private_data["ec"], private_data["evpec"], private_data["mu"]
            elif self.model=='He2018' or self.model=='Aldoroty2022':
                M,delta,b2 = theta
            ...
            for i in range(m):
                if self.model=='tripp':
                    ...
                    devs[i] = (MU[i] - (BMAX[i] - M - a*(PEW[i]) - b*C[i]))/(EVPEC[i]**2 + EBMAX[i]**2 + a**2*EPEW[i]+ b**2*EC[i]**2)**0.5
        ...
        if self.model=='tripp' or self.model=='salt' or self.model=='frni' or self.model=='pEW 4000':
        ...
        if self.model=='tripp':
            ...
            jac = np.array([-1, -self.input_data['pEW 4000'], -self.input_data['c']], dtype='object')
        elif self.model=='He2018':
